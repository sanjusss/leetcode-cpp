...
class FunctionTest
{
    using Method = std::function<void(T*&, const std::string&, const std::string&)>;
public:
    FunctionTest(const std::string& inputs,
        const std::string& expecteds,
        const std::string& calls,
        const std::unordered_map<std::string, Method>& methods = std::unordered_map<std::string, Method>()) :
        m_methods(methods)
    {
        initInputs(inputs);
